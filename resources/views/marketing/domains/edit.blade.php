...
@section('buttons')
    ...
    <x-buttons.list model='domain' :principalColor=$principalColor ?? null />
@endsection
...
@section('main')
    <div>
        <form action=" {{ route('domain.update', ['domain' => $domain->id]) }} " method="post">
            ...
            <input type="text" name="provider" size="60" value="{{ $domain->provider }}"><span
                class="fields"></span>
            <br>
            <label class="labels" for="">LINK DO PROVEDOR:</label>
            <input type="text" name="link_provider" size="60" value="{{ $domain->link_provider }}"><span
                class="fields"></span>
            <br>
            <label class="labels" for="">SENHA DO PROVEDOR:</label>
            <input type="text" name="provider_password" size="60" value="{{ $domain->domain_password }}"><span
                class="fields"></span>
            <br>
            <br>
            <label class="labels" for="">DATA DE VENCIMENTO:</label>
            <input type="date" name="due_date" size="20" value="{{ $domain->due_date }}"><span
                class="fields"></span>
            <br>
