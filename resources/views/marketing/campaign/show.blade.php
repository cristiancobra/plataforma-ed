...
<div>
    ...
    <br>
    <p class='labels'>
        RESPONSÁVEL:<span class='fields'>{{$campaign->user->id}}</span>
    </p>
    <br>
    ...
    <div style="text-align:right;padding: 2%">
		...
		<form  style="text-decoration: none;display: inline-block" action="{{route('campaign.send', ['campaign' => $campaign])}}" method="post">
			@csrf
			<input class="button-secondary" type="submit" value="ENVIAR">
		</form>
		<a class="btn btn-secondary" href=" {{route('campaign.edit', ['campaign' => $campaign->id])}}"  style="text-decoration: none;color: white;display: inline-block">
			<i class='fa fa-edit'></i>EDITAR
		</a>
		<a class="btn btn-secondary" href="{{route('campaign.index')}}">
			VOLTAR
		</a>
	</div>
	<br>
