...
@section('buttons')
    <x-buttons.list model='project' :principalColor=$principalColor ?? null />
@endsection
    ...
    <div>
        <form action=" {{ route('project.update', ['project' => $project->id]) }} " method="post">
            ...
            <input type="date" name="date_start" size="20"
                value="{{ date('Y-m-d', strtotime($project->date_start)) }}">
            <br>
            <label class="labels" for="">
                PRAZO FINAL:
            </label>
            <input type="date" name="date_due" size="20"
                value="{{ date('Y-m-d', strtotime($project->date_due)) }}">
		...
		{{ $project->description }}
            ...
            @if ($project->date_conclusion)
                <input type="date" name="date_conclusion" size="20"
                    value="{{ date('Y-m-d', strtotime($project->date_conclusion)) }}">
            @else
                <input type="date" name="date_conclusion" size="20">
            @endif
            ...
            <div style="text-align: right">
                <input class="btn btn-secondary" style="float:right;text-align: right;margin-left: 5px" type="submit"
                    value="SALVAR">
        </form>
