...
@section('buttons')
    ...
    <x-buttons.list model='email' :principalColor=$principalColor ?? null />
@endsection
    ...
    <div>
        ...
        <div style="text-align:right;padding: 2%">
            <form style="text-decoration: none;color: black;display: inline-block"
                action="{{ route('email.destroy', ['email' => $email->id]) }}" method="post">
                @csrf
                @method('delete')
                <input class="btn btn-danger" type="submit" value="APAGAR">
            </form>
            <form style="text-decoration: none;display: inline-block"
                action="{{ route('email.send', ['email' => $email]) }}" method="post">
                @csrf
                <input class="button-secondary" type="submit" value="ENVIAR">
            </form>
            <a class="btn btn-secondary" href=" {{ route('email.edit', ['email' => $email->id]) }} "
        ...
        <br>
        <p style="text-align: left;margin-left: 30px;color: white;font-size: 14px">* se a senha padrão tiver sido alterada
            pelo usuário, atualize a senha novamente com a <b>senha padrão</b>. Peça para o usuário alterar sua senha no seu
            primeiro acesso.</p>
    </div>
@endsection
