...
@section('form_start')
    ...
    @section('name')
        NOME:
        <input type='text' name='name' size='60' style='margin-left: 10px'
            value='{{ $invoice->proposal->name }}'>
    @endsection
            ...
            <div class='row mt-1'>
                ...
                <div class='cel col-2 justify-content-end' style='font-weight: 600;color:{{ $principalColor }}'>
                    @if ($invoice->totalPrice >= 0)
                        <input type='decimal' id='totalPrice' maxlength='11' onkeyup="formatCurrencyReal('totalPrice')"
                            name='totalPrice' value='{{ formatCurrency($invoice->totalPrice) }}'
                            style='text-align:right;width: 150px'>
                    @else
                        <input type='decimal' id='totalPrice' maxlength='11' onkeyup="formatCurrencyReal('totalPrice')"
                            name='totalPrice' value='{{ formatCurrency($invoice->totalPrice * -1) }}'
                            style='text-align:right;width: 150px'>
                    @endif
                </div>
            </div>
