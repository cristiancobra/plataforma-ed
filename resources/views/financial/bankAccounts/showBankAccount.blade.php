...
@section('buttons')
    ...
    <x-buttons.list model='bankAccount' :principalColor=$principalColor ?? null />
@endsection
...
@section('main')
    ...
    <div>
        ...
        <label class='labels' for=''>BANCO: </label> {{ $bankAccount->bank->name }} - código
        {{ $bankAccount->bank->bank_code }}
        <br>
        <label class='labels' for=''>DATA DE ABERTURA: </label>
        {{ date('d/m/Y', strtotime($bankAccount->date_creation)) }}
        <br>
        ...
        <br>
        <label class='labels' for=''>SALDO INICIAL: </label>
        ...
        <div style='text-align:right;padding: 2%'>
            <form style='text-decoration: none;display: inline-block'
                action='{{ route('bankAccount.destroy', ['bankAccount' => $bankAccount]) }}' method='post'>
                @csrf
                @method('delete')
                <input class='btn btn-danger' type='submit' value='APAGAR'>
            </form>
            <a class='btn btn-secondary' href='{{ route('bankAccount.edit', ['bankAccount' => $bankAccount]) }}'
            ...
            <div class='row'>
                ...
                <div class='tb col-4'>
                    @if (isset($transaction->invoice->opportunity))
                        <a
                            href=' {{ route('opportunity.show', ['opportunity' => $transaction->invoice->opportunity_id]) }}'>
                ...
                <div class='tb col-2'>
                    @if (isset($transaction->invoice->company->name))
                        <a class='white'
                            href=' {{ route('company.show', ['company' => $transaction->invoice->company->id]) }}'>
                            {{ $transaction->invoice->company->name }}
                        </a>
                    @elseif(isset($transaction->invoice->contact->name))
                        <a class='white'
                            href=' {{ route('contact.show', ['contact' => $transaction->invoice->contact->id]) }}'>
