...
@section('buttons')
    ...
    @if ($task->status == 'fazendo')
        <form style='text-decoration: none;color: black;display: inline-block'
            action=" {{ route('task.complete', ['task' => $task]) }} " method="post">
            @csrf
            @method('put')
            <button id='' class='circular-button secondary' title='Encerrar tarefa com a data atual'
...
@section('main')
    ...
    <div class='row show-label-large col-12' style='margin-top: 50px'>
        <div class=''>
            IMAGENS
        </div>
    </div>
    ...
    <div class='row' style='margin-top: 10px;text-align: right'>
        <div class='col-12'style='text-align: right'>
            <form action="{{ route('image.store') }}" method="post" enctype='multipart/form-data'>
                ...
                <input type='hidden' name='image_name' value='{{ $task->name }}'>
                <input type='file' name='image' id="imageInput" accept="image/*">
                <button type='submit' id="imageSaveBtn" class='circular-button primary' style="display: none;">
    ...
    {{-- script para exibir botao de salvamento em IMAGENS  --}}
    <script>
        document.getElementById('imageInput').addEventListener('change', function() {
            if (this.files.length > 0) {
                document.getElementById('imageSaveBtn').style.display = 'inline-block';
            } else {
    ...
    <div class='row' style='margin-top: 10px;text-align: right'>
        <div class='col-12'style='text-align: right'>
            <form id="attachmentForm" action=" {{ route('attachment.store') }} " method="post"
                enctype='multipart/form-data'>
                @csrf
                <input type='hidden' name='task_id' value='{{ $task->id }}'>
                <input type='hidden' name='attachment_name' value='{{ $task->name }}'>
                <input type='file' name='attachment' id="attachmentInput" accept=".pdf">
                <button type='submit' id="attachmentSaveBtn" class='circular-button primary' style="display: none;">
                    <i class='fa fa-save' aria-hidden='true'></i>
                </button>
            </form>
        </div>
    ...
    <script>
        document.getElementById('attachmentInput').addEventListener('change', function() {
            if (this.files.length > 0) {
                document.getElementById('attachmentSaveBtn').style.display = 'inline-block';
            } else {
                document.getElementById('attachmentSaveBtn').style.display = 'none';
            }
        });
    </script>
    ...
    @endforeach
    <div class='row'>
        <div class='tb tb-header col-12' style='text-align: right;padding: 5px;padding-right: 25px;font-size: 16px'
            colspan='7'>
    ...
    <div class='row' style='margin-top: 10px;text-align: right'>
        <div class='col-12'style='text-align: right'>
            <a class='circular-button primary'
                href='{{ route('journey.create', [
                    'taskName' => $task->name,
                    'taskId' => $task->id,
...
@section('workflow')
    @if (isset($openJourney))
        <div class='row'>
            <div class='col d-inline-block'>
                <form style='text-decoration: none;color: black;display: inline-block'
                    action="{{ route('journey.completeFromTask', ['journey' => $openJourney]) }}" method="post">
                    @csrf
                    @method('put')
                    @if ($openJourney->task_id == $task->id)
                        <button id='' class=' workflow-button-red' title='Encerrar jornada com a hora atual'
                            type='submit'>
    ...
    @else
        <div class='row mt'>
            <div class='col d-inline-block'>
                <form style='text-decoration: none;color: black;display: inline-block'
                    action="{{ route('journey.storeFromTask', ['taskId' => $task->id]) }}" method="post">
                    @csrf
                    @method('put')
                    <button id='' class='workflow-button-green' title='Iniciar com a data atual'
                        type='submit'>
                        <i class="fas fa-stopwatch" style="font-size:30px; color:white;padding-bottom: 10px"></i>
                        <br>
                        INICIAR JORNADA
