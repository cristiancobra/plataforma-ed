<section class='container frame mt-5' id='transactions' style="border-color:{{ $transactionFrameColor }}">
    <div class="row">
        <div class='col-10 pt-4 pb-3'
            style='font-size: 24px;padding-left: 5px;font-weight:600;color:{{ $transactionFrameColor }}'>
            <i class="fa fa-receipt"></i>
            PAGAMENTOS
        </div>
        <div class='col-2 d-flex justify-content-end pt-4 pb-3 text-end'
            style='font-size: 16px;padding-left: 5px;font-weight:600;color:{{ $transactionFrameColor }}'>
            <button id='paymentButtonOnOff' class='form-button'
                style="
                    color: {{ $principalColor }};
                    background-color: {{ $oppositeColor }};
                    border-color: {{ $principalColor }};
                    "
                type='button' title='ADICIONAR PAGAMENTO'>
                    ...
                    <div class='col-10'>
                     ...
                     '>
                            <a class='stretched-link'
                                href="{{ route('transaction.show', ['transaction' => $transaction]) }}"
                                style="color:white">
                            </a>
            ...
            <div class="row mt-2">
                ...
                <div class='col-1' style='text-align:left'>
                    ...
                    @if ($invoice->type == 'receita')
                        <input type="text" name="value" style="text-align: right" size='12'
                            class='prices' onkeyup="formatCurrencyRealAll('input.prices')"
                            value="{{ formatCurrencyReal($invoice->totalPrice) }}">
                    @elseif($invoice->type == 'despesa')
                        <input type="text" name="value" style="text-align: right" size='12'
                            class='prices' onkeyup="formatCurrencyRealAll('input.prices')"
                            value="{{ formatCurrencyReal($invoice->totalPrice * -1) }}">
                    @endif
            ...
            @foreach ($deletedTransactions as $deletedTransaction)
                <div class="row">
                    <div class='col-10'>
                     ...
                     '>
                            <a class='stretched-link'
                                href="{{ route('invoice.show', ['invoice' => $deletedTransaction]) }}"
                                style="color:white">
                            </a>
                            <div class='cel col-1 justify-content-start' style="font-size: 26px;font-weight: 600">
                           ...
                           '>
                            ...
                            <div class='cel col-4 justify-content-start'>
                                FATURA {{ $deletedTransaction->identifier }}: parcela
                                {{ $deletedTransaction->number_installment }} de {{ $invoice->installment }}
                            </div>
                            <div class='cel col-2'>
                    ...
                    <div class='col-2 d-flex justify-content-center'>
                        <x-buttons.trash :object="$deletedTransaction" model="invoice" />
                    </div>
                </div>
                    ...
                    @foreach ($deletedTransaction->transactions as $transaction)
                        @if ($transaction->trash != 1)
                         ...
                         '>
                                        ...
                                        <div class='cel col-1 justify-content-start'
                                            style="font-size: 26px;font-weight: 600;color: {{ $oppositeColor }}">
                                            {{ $counterTransactions++ }}
                                            <i class="fas fa-coins"
                                                style='
                               display:block;
...
<script>
    document.getElementById('paymentButtonOnOff').addEventListener('click', function() {
        const newPaymentRow = document.getElementById('newPaymentRow');
        console.log(newPaymentRow);
        if (newPaymentRow.style.display === 'none' || newPaymentRow.style.display === '') {
            newPaymentRow.style.display = 'block';
        } else {
            newPaymentRow.style.display = 'none';
        }
    });
</script>
