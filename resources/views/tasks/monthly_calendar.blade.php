...
@section('main')
    ...
    @while ($counter <= 35)
        ...
        @if ($totalDays >= $day)
            <div class='col tb d-flex justify-content-start' style='font-size: 36px;color:{{ $oppositeColor }}'
                id='{{ $counter++ }}'>
                <div class="col-3">
                    ...
                    <div class='row justify-content-center'>
                        <a class='circular-button-mini primary' style='display: inline-block;float: right'
                            href='{{ route('task.create', ['date_due' => date("Y-m-$day")]) }}'>
                        ...
                        @if (date('Y-m-d', strtotime($task->date_due)) == date("Y-m-$day"))
                            <div class='row mb-1'>
                                <div class='col-4 text-end my-auto'
                                    style='color: {{ $principalColor }};font-weight: 600;font-size:12px'>
